...
async function editInteractionMessage(applicationId, interactionToken, payload) {
  ...
  if (!response.ok) throw new Error(result.message || `Gagal memperbarui pesan Discord (${response.status}).`);
  return result;
}
